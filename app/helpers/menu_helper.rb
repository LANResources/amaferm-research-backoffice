module MenuHelper
  def menu_link_to(label, path, options = {}, &block)
    ...
    active_if_controller, active_if_action = *options[:active_if]

    active = active_if_controller.present? ? page_matches(active_if_controller, active_if_action) : request.path == path
    active_class = active ? 'active' : ''

    content_tag :li, class: [active_class, options[:li_class]].join(' ') do
      if block_given?
        [
